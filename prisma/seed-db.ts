...
function parseAyahRanges(rangeStr: string): number[] {
...
function fetchSurah(surahNum: number): Promise<any> {
...
async function main() {
  ...
  for (const req of SURAH_REQUESTS) {
    ...
    const selectedAyahs = ayahs.filter((a: any) => wantedAyahs.includes(a.numberInSurah));

    const arabicText = selectedAyahs.map((a: any) => {
